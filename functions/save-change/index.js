...
module.exports = async function (context, queueItem) {
    context.log(`[save-change] JavaScript Service Bus trigger function called for message queueItem ${queueItem}, type: ${typeof queueItem} `);

    const item = JSON.parse(queueItem);
    const operationIdOverride = { "ai.operation.id": item.operation_id };
    client.trackEvent({ name: "save-change", tagOverrides: operationIdOverride, properties: { item: queueItem } });
